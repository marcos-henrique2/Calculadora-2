...
def to_float(value_str):
    """Converte uma string (com ponto ou vírgula) para um número float padrão."""
    try:
...
def parse_time_to_decimal(time_str):
    """Converte uma string de tempo no formato H,M ou H.M para horas decimais."""
    try:
...
def calculate_cost(
    ...
    errors = []
    if filament_price <= 0 and weight > 0:
        errors.append("Preço do filamento deve ser > 0")
    if weight <= 0:
        errors.append("Filamento usado (g) deve ser > 0")
    if errors:
        return None, errors
    ...
    depreciation_cost = (printer_value / depr_hours) * \
        print_hours if depr_hours > 0 else 0.0
    ...
    # Preço antes da taxa
    price_before_fee = cost_with_complexity * (1 + (margin_pct / 100))

    # CÁLCULO DA TAXA: A taxa é calculada sobre o preço final de venda.
    # A fórmula correta para embutir a taxa é PreçoFinal = PreçoBase / (1 - TaxaPercentual)
    price = price_before_fee / \
        (1 - (additional_fee_pct / 100)
         ) if additional_fee_pct < 100 else price_before_fee
    ...
    breakdown = {
        ...
        "Taxa Adicional": price - price_before_fee  # Guardamos o valor da taxa
    }
...
st.info("No campo de tempo, use vírgula para separar horas e minutos (ex: 2,30 para 2h 30min).")
st.write("---")
...
c1, c2 = st.columns(2)
with c1:
    filament_price_str = st.text_input(
        "Preço do filamento (R$/kg)", value="100,00")
...
with c2:
    ...
    printer_value_str = st.text_input(
        "Valor da impressora (R$)", value="1500,00")
    depr_hours_str = st.text_input("Vida útil estimada (h)", value="5000,0")

st.subheader("Seu Trabalho e Estratégia")
c3, c4 = st.columns(2)
with c3:
    labor_rate_str = st.text_input(
        "Valor da sua hora de trabalho (R$/h)", value="40,00")
    active_work_hours_str = st.text_input("Tempo de Trabalho Ativo (h)", value="0,30",
                                          help="Tempo gasto preparando o arquivo, a impressora, removendo suportes, limpando a peça, etc.")
    post_processing_cost_str = st.text_input(
        "Custo Adicional de Acabamento (R$)", value="0,00")
with c4:
    maintenance_cost_str = st.text_input(
        "Custo de Manutenção (R$/h)", value="0,05")
    failure_rate_str = st.text_input("Taxa de Falha (%)", value="10,0")
    complexity_options = {"Simples (1.0x)": 1.0, "Moderado (1.25x)": 1.25,
                          "Complexo (1.5x)": 1.5, "Extremo (2.0x)": 2.0}
    complexity_label = st.selectbox(
        "Complexidade da Peça", options=list(complexity_options.keys()))
    margin_pct_str = st.text_input("Margem de lucro (%)", value="100,0")

# NOVO CAMPO OPCIONAL PARA TAXAS
st.subheader("Taxas de Venda (Opcional)")
additional_fee_str = st.text_input("Taxa Adicional (ex: Shopee, %)", value="0,00",
                                   help="Digite a porcentagem da taxa que o marketplace cobra. Ex: 20 para 20%.")
...
if st.button("Calcular (Apenas para Conferência)"):
    ...
    additional_fee_pct = to_float(additional_fee_str)  # Converte a nova taxa
    ...
    result, errors = calculate_cost(
        ...
        printer_value, depr_hours, margin_pct,
        maintenance_cost_per_hour, failure_rate_pct, post_processing_cost, complexity_multiplier,
        additional_fee_pct  # Passa a nova taxa para a função
    )
    if errors:
        st.session_state.local_calc = None
        for err in errors:
...
if st.session_state.local_calc:
    st.write("---")
    st.subheader(
        f"Resultado para: '{st.session_state.local_calc['item_name']}'")
    result = st.session_state.local_calc
    breakdown = result["breakdown"]
    total_cost = result["total_cost"]
    price = result["price"]

    # Exibe o valor da taxa, se houver
    if breakdown['Taxa Adicional'] > 0:
        st.write(
            f"**Taxa de Venda Embutida (un.): R$ {breakdown['Taxa Adicional']:.2f}**")

    st.write(f"**Preço sugerido de Venda (un.): R$ {price:.2f}**")
    st.write(
        f"**Preço Total ({result.get('quantity', 1)} un.): R$ {result.get('final_price', 0):.2f}**")
    st.write("---")

    with st.expander("Ver composição detalhada dos custos"):
        if breakdown['Material (c/ falha)'] > 0:
            st.write(
                f"- Material (c/ ajuste de falha): R$ {breakdown['Material (c/ falha)']:.2f}")
...
if st.session_state.quote_items:
    ...
    total_quote_price = sum(item['final_price']
    ...
    for i, item in enumerate(st.session_state.quote_items):
        ...
        col1.markdown(
            f"- ({item['quantity']}x) **{item['item_name']}** (R$ {item['final_price']:.2f})")
